...
{{ $header->setMetaTitle('Announcement - Add Document') }}
@if($mode == 'add')
{{ $header->setPageTitle(trans('site/announcement.document.add_head')) }}

@else
{{ $header->setPageTitle(trans('site/announcement.document.edit_head')) }}
@endif
<?php
...
?>
@section('breadcrumb')
    <li><a href="{{ URL::to('announcement/list-document') }}">{{ trans('site/announcement.breadcrumb.list_document') }}</a> <span class="divider">/</span></li>
    <li class="active">{{ trans('site/announcement.breadcrumb.manage_document') }}</li>
@stop
@section('content')
     ...
     {{ Form::model($details, array('id' => 'submitentry', 'name' => 'submitentry', 'files' => 'true', 'class' => "stdform" )) }}
            ...
            {{ Form::hidden('submit_status', null, array('id' => 'submit_status')) }}
           	<div class="par control-group {{{ $errors->has('topic') ? 'error' : '' }}}">
           	    {{ Form::label('topic', trans('site/announcement.document.topic'), array('class' => 'required-icon')) }}
                <div class="controls">
            ...
            <div class="par control-group {{{ $errors->has('category_id') ? 'error' : '' }}}">
                {{ Form::label('category_id', trans('site/announcement.document.category'), array('class' => 'required-icon')) }}
                <div class="controls">
                    {{  Form::select('category_id', array('' => 'select') + $dd_arr['category_names']); }}
                     <label for="topic" class="error" generated="true">{{{ $errors->first('topic') }}}</label>
                </div>
            </div>
            ...
            <div class="par control-group {{{ $errors->has('description') ? 'error' : '' }}}">
                {{ Form::label('description', trans('site/announcement.document.description')) }}
                <div class="controls">
            ...
            <div class="par control-group">
               {{ Form::label('add_to', trans('site/announcement.document.published_to')) }}
               <div class="controls">
               <span class="field">
                  {{ Form::checkbox('published_to_admin', '1', null, array('id' => 'published_to_admin', 'name' => 'published_to_admin', 'class' => "input-large")) }}
                  {{ Form::label('published_to_admin', trans('site/announcement.document.published_to_admin') )}}
               </span>
               <span class="field">
                  ...
                  {{ Form::label('published_to_supervisor', trans('site/announcement.document.published_to_supervisor') )}}
               </span>
               <span class="field">
                  ...
                  {{ Form::label('published_to_all_employees', trans('site/announcement.document.published_to_all_employees') )}}
               </span>
               </div>
            </div>
            <div class="par control-group {{{ $errors->has('date_published') ? 'error' : '' }}}">
                {{ Form::label('date_published', trans('site/announcement.document.date_published')) }}
                <div class="controls">
             ...
             <p class="stdformbutton">
                  	<button name="draft" id="fn_draftbtn" class="btn btn-success" value="draft">{{ trans('site/announcement.document.save_as_draft') }}</button>
                  	<button name="publish" id="fn_submitbtn" class="btn btn-success" value="published">{{ trans('site/announcement.document.publish') }}</button>
...
<div class="pagetitle">
     ...
     {{ Form::open(array('id' => 'listFrm', 'name' => 'listFrm', 'class' => "stdform", 'method' => 'post', 'url' => 'announcement/delete-document-attachment')) }}
             ...
             <tbody>
                ...
                ?>
                  <tr>
                    ...
                    <td><a href="{{ Url::to('announcement/add-document-attachment?id='.$record['id']) }}">{{ $record['orig_file_name'] }}</a></td>
       ...
       {{ Form::hidden('document_id', $details['id']) }}
     {{ Form::close() }}
          <button class="btn btn-danger" onClick="location.href='{{ Url::to('announcement/add-document-attachment?document_id='.$details['id']) }}'">{{ trans('general.add') }}</button>
...
<script>
    ...
    var viewListUrl = '{{ URL::to('announcement/list-document'); }}';
    ...
    $( document ).ready(function()
    {
       ...
       $("#submitentry").validate({
          	rules: {
          		topic: {
          			required: true,
          			maxlength: 100,// {{ config::get('site.document_topic_max_length') }}
          		}
          	},
            messages: {
                topic: {
            ...
            submitHandler: function(form) {
                var action_btn = ($('#submit_status').val() == 'draft') ? 'fn_draftbtn' : 'fn_submitbtn'
                $("#"+action_btn).text("{{ trans('general.saving')}}").attr("disabled", true);
                form.submit();
            }
       });
