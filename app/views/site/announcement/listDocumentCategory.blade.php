...
{{ $header->setPageTitle(trans('site/announcement.document_category_list.list_page_title_head')) }}
<?php
    $left_main_menu_id = 'left_main_admin';
    $left_menu_id_level1 = 'left_level1_announcement';
?>
@section('breadcrumb')
    <li class="active">{{ trans('site/announcement.breadcrumb.list_document_category') }}</li>
@stop
@section('content')
 <div class="widgetcontent">
   <div class="widgetcontent">
     <button class="btn btn-warning" onClick="location.href='{{ Url::to('announcement/add-document-category') }}'">{{ trans('general.add') }}</button>
        ...
        <table class="table table-bordered">
            ...
            <thead>
                <tr id="sortTableHead">
                    ...
                    <th class="head0 sort-cursor" id="name" title="{{ trans('general.sortby').' '. trans('site/announcement.document_category_list.list_col_head_name')}}">
                        {{ trans('site/announcement.document_category_list.list_col_head_name') }}
             ...
             <tbody>
                @foreach($details as $record)
                  <tr>
                    ...
                    <td><a href="{{ Url::to('announcement/add-document-category?id='.$record['id']) }}">{{ $record['name'] }}</a></td>
                  </tr>
                @endforeach
                @if(!count($details))
                    <tr>
...
<script>
    var sort_url = '{{ URL::to('announcement/list-document-category'); }}';
