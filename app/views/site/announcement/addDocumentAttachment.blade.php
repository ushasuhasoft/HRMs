...
{{ $header->setMetaTitle('Document - Add Attachment') }}
@if($mode == 'add')
    {{ $header->setPageTitle(trans('site/announcement.document_attachment.add_title_head')) }}
@else
     {{ $header->setPageTitle(trans('site/announcement.document_attachment.edit_title_head')) }}
@endif
<?php
...
@section('breadcrumb')
   ...
   <li><a href="{{ URL::to('announcement/add-document?id='.$document_id) }}">{{ trans('site/announcement.breadcrumb.manage_document') }}</a> <span class="divider">/</span></li>
     ...
     {{ Form::model($details, array('id' => 'submitentry', 'name' => 'submitentry', 'files' => 'true', 'class' => "stdform" )) }}
           ...
           @if($mode == 'add')
             <div class="par control-group {{{ $errors->has('attachment_file') ? 'error' : '' }}}">
                {{ Form::label('attachment_file', trans('site/announcement.document_attachment.attachment_file')) }}
                <div class="controls">
                   {{  Form::file('attachment_file'); }}
                   <span class="muted-text">{{ trans('site/announcement.document_attachment.attachment_max_file_size',  array('max_size' => $dd_arr['max_file_size'])); }}</span>
           ...
           @else
             <div class="par control-group {{{ $errors->has('attachment_file') ? 'error' : '' }}}"
                ...
                <div class="controls">
                    <a href="{{ Url::to('announcement/download-document-attachment')}}?attachment_id={{$details['id']}}" target="_blank">{{ $details['orig_file_name'] }}</a> <br />
                    <ul class="unstyled">
                        <li class="inline">
                            {{ Form::radio('update_file', 'current', true, array('id' => 'update_file_current', 'name' => 'update_file')) }}
                            {{ Form::label('update_file_current', trans('site/announcement.document_attachment.file_keep') )}}
                        </li>
                        <li class="list-inline">
                            {{ Form::radio('update_file', 'replace',  false, array('id' => 'update_file_replace', 'name' => 'update_file')) }}
                            {{ Form::label('update_file_replace', trans('site/announcement.document_attachment.file_replace') )}}
                        </li>
                    </ul> <br />
            ...
            <div class="par control-group {{{ $errors->has('description') ? 'error' : '' }}}">
                {{ Form::label('description', trans('site/announcement.document_attachment.description')) }}
                <div class="controls">
...
<script>
    ...
    var viewListUrl = '{{ URL::to('announcement/add-document?id=',$document_id); }}';
    var mode = '{{ $mode }}';
    ...
    $( document ).ready(function()
    {
       $("#submitentry").validate({
          	rules: {
                ...
                description: {
                    maxlength: {{ config::get('site.document_attachment_desc_max_length') }}
                }
          	},
            messages: {
                attachment_file: {
