...
{{ $header->setPageTitle(trans('site/organization.nationality_list.list_page_title_head')) }}
<?php
    $left_main_menu_id = 'left_main_admin';
    $left_menu_id_level1 = 'left_level1_nationality';
?>
@section('breadcrumb')
    <li class="active">{{ trans('site/organization.breadcrumb.list_nationality') }}</li>
@stop
@section('content')
 <div class="widgetcontent">
   <div class="widgetcontent">
     <button class="btn btn-warning" onClick="location.href='{{ Url::to('organization/add-nationality') }}'">{{ trans('general.add') }}</button>
        ...
        <table class="table table-bordered">
            ...
            <thead>
                <tr id="sortTableHead">
                    ...
                    <th class="head0 sort-cursor" id="name" title="{{ trans('general.sortby').' '. trans('site/organization.nationality_list.list_col_head_name')}}">
                        {{ trans('site/organization.nationality_list.list_col_head_name') }}
             ...
             <tbody>
                @foreach($details as $record)
                  <tr>
                    ...
                    <td><a href="{{ Url::to('organization/add-nationality?id='.$record['id']) }}">{{ $record['name'] }}</a></td>
                  </tr>
                @endforeach
                @if(!count($details))
                    <tr>
...
<script>
    var sort_url = '{{ URL::to('organization/list-nationality'); }}';
