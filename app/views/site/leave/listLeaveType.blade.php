...
{{ $header->setMetaTitle('Leave Types') }}
{{ $header->setPageTitle(trans('site/leave.leave_type.list_page_title_head')) }}
<?php
    $left_main_menu_id = 'left_main_leave';
    $left_menu_id_level1 = 'left_level1_configuration';
?>
@section('breadcrumb')
    <li class="active">{{ trans('site/leave.breadcrumb.config_list_leave_type') }}</li>
@stop
@section('content')
 <div class="widgetcontent">
   <div class="widgetcontent">
     <button class="btn btn-warning" onClick="location.href='{{ Url::to('leave-config/add-leave-type') }}'">{{ trans('general.add') }}</button>
        ...
        <table class="table table-bordered">
            ...
            <thead>
                <tr id="sortTableHead">
                    ...
                    <th class="head0 sort-cursor" id="name" title="{{ trans('general.sortby').' '. trans('site/leave.leave_type.list_col_head_name')}}">
                        {{ trans('site/leave.leave_type.list_col_head_name') }}
             ...
             <tbody>
                @foreach($details as $record)
                  <tr>
                    ...
                    <td><a href="{{ Url::to('leave-config/add-leave-type?id='.$record['id']) }}">{{ $record['name'] }}</a></td>
                  </tr>
                @endforeach
                @if(!count($details))
                    <tr>
...
<script>
    var sort_url = '{{ URL::to('leave-config/list-leave-type'); }}';
