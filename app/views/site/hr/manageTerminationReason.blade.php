...
{{ $header->setMetaTitle('Manage Termination Reason') }}
{{ $header->setPageTitle(trans('site/hr.manage_termination_reason.list_page_title_head')) }}
<?php
    $left_main_menu_id = 'left_main_hr';
    $left_menu_id_level1 = 'left_level1_configuration';
?>
@section('breadcrumb')
    <li class="active">{{ trans('site/hr.breadcrumb.manage_termination_reasons') }}</li>
@stop
@section('content')
 <div class="row-fluid">
      <div id="fnLoading" style="display: none"> Loading ... </div>
      <div class="widgetcontent" id="addForm" style="display: none">
          <h3 id="form_manage_head">{{ trans('site/hr.manage_termination_reason.add_head') }}</h3>
          {{ Form::open(array('id' => 'submitentry', 'name' => 'submitentry', 'method' => 'post', 'class' => "stdform" )) }}
                 ...
                 {{ Form::hidden('action', 'save') }}
                	<div class="par control-group {{{ $errors->has('name') ? 'error' : '' }}}">
                	    {{ Form::label('name', trans('site/hr.manage_termination_reason.name'), array('class' => 'required-icon')) }}
                     <div class="controls">
                         {{  Form::text('name', null); }}
      ...
      <table class="table table-bordered">
                  ...
                  <thead>
                      <tr id="sortTableHead">
                          ...
                          <th class="head0 sort-cursor" id="name" title="{{ trans('general.sortby').' '. trans('site/hr.manage_termination_reason.list_col_head_name')}}">
                              {{ trans('site/hr.manage_termination_reason.list_col_head_name') }}
 ...
 <script>
         ...
         $("#removeSelected").click(function(e) {
          ...
          $('#btnAdd').click(function() {
           ...
           @if(count($errors) && Input::old('action') == 'save' )
                ...
                @if( Input::old('id'))
                    $('#form_manage_head').html('{{ trans('site/hr.manage_termination_reason.edit_head') }}');
                @else
                    $('#form_manage_head').html('{{ trans('site/hr.manage_termination_reason.add_head') }}');
                @endif
                $('#id').val(id);
                $('#name').val(name);
                $('#addForm').show();
    ...
    function editRecord(id)
    {
        ...
        $('#fnLoading').show();
        $('#fn_btnholder').hide();
        var url = '{{ URL::to('hr-config/termination-reason-info')}}?id='+id;
