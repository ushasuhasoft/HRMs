...
{{ $header->setPageTitle(trans('site/jobData.employment_status_list.list_page_title_head')) }}
<?php
...
@section('content')
 <div class="widgetcontent">
   <div class="widgetcontent">
     <button class="btn btn-warning" onClick="location.href='{{ Url::to('job/add-employment-status') }}'">Add</button>
        ...
        <table class="table table-bordered" id="dyntable123">
            ...
            <thead>
                <tr id="sortTableHead">
                    ...
                    <th class="head0 sort-cursor" id="name" title="{{ trans('general.sortby').' '. trans('site/jobData.employment_status_list.list_col_head_name')}}">
                        {{ trans('site/jobData.employment_status_list.list_col_head_name') }}
             ...
             <tbody>
                @foreach($details as $record)
                  <tr>
                    ...
                    <td><a href="{{ Url::to('job/add-employment-status?id='.$record['id']) }}">{{ $record['name'] }}</a></td>
                  </tr>
                @endforeach
                @if(!count($details))
                    <tr>
...
<script>
    ...
    var sort_url = '{{ URL::to('job/list-employment-status'); }}';
