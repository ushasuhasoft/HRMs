...
{{ $header->setMetaTitle('Manage Qualification Skill') }}
@if($details['id'])
     {{ $header->setPageTitle(trans('site/qualificationData.skill.edit_head')) }}
@else
    {{ $header->setPageTitle(trans('site/qualificationData.skill.add_head')) }}
@endif
<?php
...
?>
@section('breadcrumb')
    <li><a href="{{ URL::to('qualification/list-skill') }}">{{ trans('site/qualificationData.breadcrumb.list_skill') }}</a> <span class="divider">/</span></li>
    <li class="active">{{ trans('site/qualificationData.breadcrumb.add_skill') }}</li>
@stop
@section('content')
     ...
     {{ Form::model($details, array('id' => 'submitentry', 'name' => 'submitentry', 'files' => 'true', 'class' => "stdform" )) }}
            {{ Form::hidden('id') }}
           	<div class="par control-group {{{ $errors->has('name') ? 'error' : '' }}}">
           	    {{ Form::label('name', trans('site/qualificationData.skill.name'), array('class' => 'required-icon')) }}
                <div class="controls">
                    {{  Form::text('name', null); }}
...
<script>
    ...
    var viewListUrl = '{{ URL::to('qualification/list-skill'); }}';
    ...
    $( document ).ready(function()
    {
       $("#submitentry").validate({
          	rules: {
          		name: {
          			required: true,
          			maxlength: {{ Config::get('site.qualification_skill_name_max_length') }},
          			uniqueName:true
          		}
          	},
            messages: {
