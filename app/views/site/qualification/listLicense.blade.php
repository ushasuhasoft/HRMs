...
{{ $header->setPageTitle(trans('site/qualificationData.license_list.list_page_title_head')) }}
<?php
    $left_main_menu_id = 'left_main_admin';
    $left_menu_id_level1 = 'left_level1_qualification';
?>
@section('breadcrumb')
    <li class="active">{{ trans('site/qualificationData.breadcrumb.list_license') }}</li>
@stop
@section('content')
 <div class="widgetcontent">
   <div class="widgetcontent">
     <button class="btn btn-warning" onClick="location.href='{{ Url::to('qualification/add-license') }}'">{{ trans('general.add') }}</button>
        ...
        <table class="table table-bordered">
            ...
            <thead>
                <tr id="sortTableHead">
                    ...
                    <th class="head0 sort-cursor" id="name" title="{{ trans('general.sortby').' '. trans('site/qualificationData.license_list.list_col_head_name')}}">
                        {{ trans('site/qualificationData.license_list.list_col_head_name') }}
             ...
             <tbody>
                @foreach($details as $record)
                  <tr>
                    ...
                    <td><a href="{{ Url::to('qualification/add-license?id='.$record['id']) }}">{{ $record['name'] }}</a></td>
                  </tr>
                @endforeach
                @if(!count($details))
                    <tr>
