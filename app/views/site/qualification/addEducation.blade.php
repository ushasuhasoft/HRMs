...
@if($details['id'])
     {{ $header->setPageTitle(trans('site/qualificationData.education.edit_head')) }}
@else
    {{ $header->setPageTitle(trans('site/qualificationData.education.add_head')) }}
@endif
<?php
...
?>
@section('breadcrumb')
    <li><a href="{{ URL::to('qualification/list-education') }}">{{ trans('site/qualificationData.breadcrumb.list_education') }}</a> <span class="divider">/</span></li>
    <li class="active">{{ trans('site/qualificationData.breadcrumb.add_education') }}</li>
@stop
@section('content')
     ...
     {{ Form::model($details, array('id' => 'submitentry', 'name' => 'submitentry', 'class' => "stdform" )) }}
            {{ Form::hidden('id') }}
           	<div class="par control-group {{{ $errors->has('name') ? 'error' : '' }}}">
           	    {{ Form::label('name', trans('site/qualificationData.education.name'), array('class' => 'required-icon')) }}
                <div class="controls">
                    {{  Form::text('name', null); }}
...
<script>
    ...
    var viewListUrl = '{{ URL::to('qualification/list-education'); }}';
    ...
    $( document ).ready(function()
    {
       $("#submitentry").validate({
          	rules: {
          		name: {
          			required: true,
          			maxlength: {{ Config::get('site.qualification_education_name_max_length') }},
          			uniqueName:true
          		}
          	},
            messages: {
