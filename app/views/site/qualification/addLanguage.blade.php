...
@if($details['id'])
     {{ $header->setPageTitle(trans('site/qualificationData.language.edit_head')) }}
@else
    {{ $header->setPageTitle(trans('site/qualificationData.language.add_head')) }}
@endif
<?php
...
?>
@section('breadcrumb')
    <li><a href="{{ URL::to('qualification/list-language') }}">{{ trans('site/qualificationData.breadcrumb.list_language') }}</a> <span class="divider">/</span></li>
    <li class="active">{{ trans('site/qualificationData.breadcrumb.add_language') }}</li>
@stop
@section('content')
     ...
     {{ Form::model($details, array('id' => 'submitentry', 'name' => 'submitentry', 'files' => 'true', 'class' => "stdform" )) }}
            {{ Form::hidden('id') }}
           	<div class="par control-group {{{ $errors->has('name') ? 'error' : '' }}}">
           	    {{ Form::label('name', trans('site/qualificationData.language.name'), array('class' => 'required-icon')) }}
                <div class="controls">
                    {{  Form::text('name', null); }}
...
<script>
    ...
    var viewListUrl = '{{ URL::to('qualification/list-language'); }}';
    ...
    $( document ).ready(function()
    {
       $("#submitentry").validate({
          	rules: {
          		name: {
          			required: true,
          			maxlength: {{ Config::get('site.qualification_language_name_max_length') }},
          			uniqueName:true
          		}
          	},
            messages: {
